...
e.drop <- function(raw_matrix_counts, project.name, saveDir){
  ...
  seurat.ok <- CreateSeuratObject(counts = counts.ok, project = as.character(project.name))
  seurat.empty <- CreateSeuratObject(counts = counts.empty, project = as.character(project.name))
  ...
  QC_plots_emptydrop(seuratObj.ok = seurat.ok, seuratObj.empty = seurat.empty, pro.name = project.name, low = 100, FDR = 0.05, saveDir = saveDir)
  emptydrop_hist <- hist(e.out$PValue[e.out$Total >= 100], breaks = 20, xlab="P-value", main = "", col="grey80")
  print(plot(emptydrop_hist))
  print("P-Value histogram should be uniform")
